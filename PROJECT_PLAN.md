...
|----|---------|----------|--------|-----------|
| MUT-mean | Mean over atoms → `(39,)` | Euclidean | **implemented** | Simplest sensible baseline for the strain — coefficients are homogeneous (same basis), L2 respects irrep-norm magnitudes as real chemical signal, mean implicitly weights elements by stoichiometry (the H:C ratio shift across an alkane series is what gives strong M-MONO/M-LIN). |
| MUT-mean-cosine | Mean over atoms → `(39,)` | Cosine | **implemented** | A/B against MUT-mean to test whether magnitude carries signal or washes out. Observed: M-HAMMETT-ABS ≈ 2× better (direction matters for substituent-orientation tasks); M-SMOOTH and silhouettes worse (magnitude carries molecule-size signal). Cross-experiment rank essentially tied with MUT-mean. |
...
| MUT-mean-max | Max (componentwise) over atoms → `(39,)` | Euclidean | **implemented (negative control, confirmed)** | Tests the additivity assumption of mean-pooling. Confirmed worse on all experiments (cross-exp rank 11.78 vs 8.72 for MUT-mean). M-MONO/M-LIN on homologous series collapse to ~0.15/0.21 — replacing additive pooling with "any atom with extreme feature X" loses all the smoothness and discriminative signal. Keep as a permanent reference point for "this is what destroying additivity looks like". |
| MUT-mean-perelement | Mean within each element type (H, C, N, O, F, S, Cl), concat → 7×39 = 273-d (missing elements get zero blocks) | Euclidean | **implemented (negative control, confirmed)** | Originally framed as "picks up composition signal"; *empirically the opposite*. Per-element mean removes the implicit stoichiometric weighting that whole-molecule mean carries (e.g. H:C ratio shift across CₙH₂ₙ₊₂). M-HAMMETT-PAIR goes anti-correlated (−0.107); cross-exp rank 11.83 vs 8.72 for MUT-mean. Use `MUT-mean-perelement-sum` for an actual composition-preserving variant. |
| MUT-mean-perelement-sum | Sum within each element type (same layout as above), concat → 7×39 = 273-d | Euclidean | planned | The composition-preserving variant `MUT-mean-perelement` failed to be. Per-element sum equals per-element-mean × element-count, so each block carries both per-element feature info *and* the count of atoms of that element. Direct test of whether composition (in the stoichiometric sense) adds signal over plain `MUT-mean`. |

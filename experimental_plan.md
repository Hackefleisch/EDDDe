...
|------------|------------|----------|------------|
| **MUT-mean** | Mean over atoms → `(39,)` | Euclidean | Refactored from the original 127-d raw mean (rotation-leaky); now scalarised per the contract above. Strongest variant in the family so far — cross-experiment rank 8.72/15, best in Strain A on homologous smoothness. |
| **MUT-mean-cosine** | Mean over atoms → `(39,)` | Cosine | Direct A/B against MUT-mean. Empirically: direction carries Hammett-style substituent signal (M-HAMMETT-ABS ≈ 2× better), magnitude carries molecule-size signal (M-SMOOTH worse). Cross-rank tied with MUT-mean. |
...
| **MUT-mean-max** | Max over atoms → `(39,)` | Euclidean | Control, confirmed negative: cross-exp rank 11.78 vs 8.72 for mean. Replacing additive pooling with "any atom with extreme feature X" collapses M-MONO/M-LIN on homologous series (~0.15/0.21). The additivity assumption of mean-pooling is doing real work. Permanent reference point for "this is what destroying additivity looks like." |
| **MUT-mean-perelement** | Mean within each element (H, C, N, O, F, S, Cl), concat → 273-d | Euclidean | Originally framed as a composition-preserving variant; empirically the opposite. Per-element *mean* strips out the stoichiometric weighting that whole-molecule mean carries implicitly — the H:C ratio shift across an alkane series is what gives MUT-mean its M-MONO/M-LIN strength, and splitting by element removes it. M-HAMMETT-PAIR goes anti-correlated (−0.107); cross-rank 11.83. Use **MUT-mean-perelement-sum** for an actual composition-preserving baseline. |
| **MUT-mean-perelement-sum** | Sum within each element (same layout) → 273-d | Euclidean | What `MUT-mean-perelement` was meant to be. Per-element sum = element-count × per-element-mean, so each block carries both per-element feature info *and* the atom count of that element. Direct test of whether composition (in the stoichiometric sense) adds signal on top of plain `MUT-mean`. |
...
- **Chemical clustering (Set A vs Set B):** In the MDS plot, do electron-donating groups (OH, NH₂, OCH₃, CH₃) cluster separately from electron-withdrawing groups (NO₂, COOH, CHO, COCH₃)? Evaluate by silhouette score after labelling compounds as donors/acceptors/neutral. **Split by scaffold**, since the chemical expectation is opposite: on aromatic Set A (conjugated) we *want* high clustering — electronic effects propagate through the π-system, so donors and acceptors should separate; on aliphatic Set B (cyclohexane, sp³) we *want* low clustering — electronic effects are localised and shouldn't produce global donor/acceptor separation. A method that scores similarly on both sets either (a) detects only local substituent identity, not electronic delocalisation, or (b) is insensitive to scaffold context. Ranking directions must therefore differ: S6 ↑, S7 ↓.
- **Hammett correlation (Set C):** Spearman ρ between |σᵢ − σⱼ| and d(Xᵢ, Xⱼ) across all pairs of Set C compounds (M-HAMMETT-PAIR). The pair formulation avoids the sign mismatch of the naive signed-σ vs distance-from-H correlation — σ_para is signed (donors negative, acceptors positive) but Euclidean distance is unsigned, so a chemically-correct U-shape around the H-compound scores poorly on the naive metric. Using |Δσ| on the left side and all C(n,2) pairs on the right makes both sides symmetric and raises statistical power from n=9 to 36 pairs. A secondary metric M-HAMMETT-ABS = Spearman ρ(|σ|, d(X, H)) is also reported as a sanity check.
...
3. For each query, rank the valid+test pool by ascending distance. Record the rank and distance of every active in the pool (inactive positions are implicit). Average retrieval metrics over queries and seeds per target.
...
**Note on k-NN classification:** WelQrate's reference protocol also includes a k-NN classification task (each test molecule's label predicted by vote of its k nearest neighbors, scored via AUC-ROC at k = 5, 10, 20). EDDDe omits this task. In sparse-active regimes (hit rate ~1 %) the k-NN AUC-ROC is dominated by the same "actives cluster near actives at the top of the ranking" signal that EF₁%, BEDROC₂₀, and LogAUC already measure, and full-mode D3 would have this single task account for ~99 % of EXP-3a compute. Phase-A retrieval metrics carry the load.

...
**Distance computation in experiments.** Experiments should never write their own pair loops. Call `pairwise_matrix(method, embeddings, query_ids, candidate_ids)` for rectangular blocks, or `pair_distances(method, embeddings, pairs, symmetrise=...)` for explicit pair lists, both from [eddde/methods/distance.py](eddde/methods/distance.py). Dispatch:

 - Batched override present (B1–B7, B9, MUT-mean) → call `method.distances(...)` directly (`pairwise_matrix` on the full rectangular block; `pair_distances` densifies the unique IDs in the pair list into a submatrix and indexes out). The per-pair benchmark is irrelevant and never consulted; vectorised C/BLAS/GPU calls don't benefit from external IPC fanout.
 - Per-pair-only (B8, future alignment / per-pair OT methods) → predict serial time as `n_calls × method._distance_time_per_pair` and fan out across `multiprocessing.Pool` iff that exceeds `_PARALLEL_MIN_SERIAL_SECONDS = 1.0`. Otherwise fall through to the inherited serial loop. `eddde.N_WORKERS` (overridable via `--num-workers`) controls the pool size. `pair_distances` evaluates only the listed pairs (×2 when `symmetrise=True`), so sparse experiments like EXP-4 do not pay a full N×N.
...
This module is intentionally under `eddde.methods` rather than `eddde.experiments.retrieval_common` because matrix-shaped and pair-list distance are shared by EXP-2, EXP-3a/3b, EXP-4, and the planned EXP-5/EXP-6 — not just retrieval. For sparse pair sets (EXP-4 cliffs, planned EXP-5 bioisostere sets) use `pair_distances(method, embeddings, pairs, symmetrise=...)` from the same module instead of densifying into a full N×N via `pairwise_matrix`.
...
**Use `pairwise_matrix` / `pair_distances` for any multi-pair distance computation.** Never write your own `for q in queries: for c in candidates: method.distance(q, c)` loop — it bypasses the batched-override fast path for B1–B7, B9, MUT-mean (and future GPU methods), and also bypasses the multiprocessing pool that makes B8 (and future alignment-style / per-pair-OT methods) tractable at full-mode scale. Both helpers live in [eddde/methods/distance.py](eddde/methods/distance.py); see "Distance computation in experiments" above for what they dispatch to. Prefer `pair_distances` when the experiment already has an explicit pair list (EXP-4 cliffs, planned EXP-5 bioisostere sets) so per-pair-only methods do not pay a full N×N; use `pairwise_matrix` for rectangular query×candidate blocks (EXP-2, EXP-3a/3b).

...
`distance_time_per_pair` (only populated on **embedding** manifests) is the mean wall-clock time of one `method.distance(e1, e2)` call, measured by [eddde/methods/base.py](eddde/methods/base.py) `benchmark_distance` immediately after embedding via a time-budgeted sample (target ~1 s, bounded `[20, 2000]` pairs, 5-call warmup, seeded). It serves two purposes: (1) it surfaces in `results/SUMMARY.md` as the `s/pair` column for fair cross-method comparison, and (2) it drives the parallel/serial dispatch in `pairwise_matrix` — see "Distance computation in experiments" below. The runner stashes the value on `method._distance_time_per_pair` in `_embed_if_stale` (loaded from manifest on cache hit, freshly measured on rebuild) so the dispatch always has a real measurement. The benchmark re-runs whenever embeddings rebuild; existing pre-benchmark manifests get a one-shot in-place upgrade in `runner._embed_if_stale` so adopting the field doesn't require cascade rebuilds. For methods that override `distances()` only (B1–B7, B9, MUT-mean), `distance()` is the inherited 1×1 wrapper — the measured `s/pair` includes ~µs of wrapping overhead. That is the right number for fair per-pair comparison since it's exactly the cost a caller pays per pair; the batched path (used by experiments) does not pay it.
...
**Distance computation in experiments.** Experiments should never write their own pair loops. Call `pairwise_matrix(method, embeddings, query_ids, candidate_ids)` from [eddde/methods/distance.py](eddde/methods/distance.py). Dispatch:

  - Batched override present (B1–B7, B9, MUT-mean) → call `method.distances(...)` directly. The per-pair benchmark is irrelevant and never consulted; vectorised C/BLAS/GPU calls don't benefit from external IPC fanout.
  - Per-pair-only (B8, future alignment / per-pair OT methods) → predict serial time as `n_pairs × method._distance_time_per_pair` and fan out across `multiprocessing.Pool` iff that exceeds `_PARALLEL_MIN_SERIAL_SECONDS = 1.0`. Otherwise fall through to the inherited serial loop. `eddde.N_WORKERS` (overridable via `--num-workers`) controls the pool size.

The per-pair cost comes from the embedding-stage benchmark: the runner stashes it on the method (loaded from manifest on cache hit, freshly measured on rebuild — see `_embed_if_stale`). If it's still missing at call time (ad-hoc usage outside the runner), `pairwise_matrix` runs `benchmark_distance` on the passed-in embeddings and stashes the result. There is no static pair-count fallback — a real measurement is always available before the parallel/serial decision.

For methods with intrinsically asymmetric per-pair distance (B8's alignment optimizer fixes one mol and rotates the other, so `d(a,b) ≠ d(b,a)`), self-distance matrices fed to MDS or paired-statistics need a `D = 0.5 * (D + D.T)` symmetrisation step in the experiment (no-op for naturally-symmetric methods). EXP-2 already does this; replicate the pattern in any future experiment that builds a square self-matrix.

This module is intentionally under `eddde.methods` rather than `eddde.experiments.retrieval_common` because matrix-shaped distance is shared by EXP-2 and the planned EXP-5/EXP-6, not just retrieval. A future `pair_distances(method, embeddings, pairs)` for explicit pair-list inputs (EXP-4 cliffs, EXP-5 pair sets) will live in the same module.
...
**MUT equivariance audit.** MUTs set `is_mut = True` as a class attribute on `Method`. That's the only structural marker — no separate base class — and EXP-EQUIVAR's `method_filter` uses it to keep baselines off the S0 fixture. The four contract metrics from PROJECT_PLAN.md §3.2.1 are exercised by **EXP-EQUIVAR** ([eddde/experiments/equivariance.py](eddde/experiments/equivariance.py)) on the **S0 fixture** ([eddde/data/sources/audit_fixture.py](eddde/data/sources/audit_fixture.py)): 5 small SMILES × 3 variants (orig / rotated by fixed `R ∈ SO(3)` / atoms permuted by fixed `P`), all 15 conformers cached through the standard ElektroNN pipeline. Distances are symmetrised (`0.5·(d(a,b)+d(b,a))`) so the experiment treats naturally-symmetric MUTs (no-op) and B8-style asymmetric ones uniformly. Metrics — all max across the 5 bases, lower better, per-base values in `raw.csv`:
- `M-EQUIVAR-ROT` — `d(emb(orig), emb(rot))`. Actual rotation distance the MUT produces; includes both MUT leakage and any ElektroNN-noise contribution propagated through the MUT.
- `M-EQUIVAR-ROT-FLOOR` — `d(emb(rot_ideal), emb(rot))` where `rot_ideal` feeds the MUT Wigner-D-rotated cached coefficients (`coeff_orig @ D(R)^T`) and `rot` feeds the MUT real ElektroNN-rerun coefficients. Both inputs describe the *same* rotated molecule; any gap at the MUT embedding is ElektroNN's noise translated through the MUT's pooling — i.e. the baseline in MUT-distance units. `ROT ≈ ROT-FLOOR` → MUT is rotation-invariant and what you see is ElektroNN noise; `ROT ≫ ROT-FLOOR` → MUT itself is leaky.
- `M-EQUIVAR-PERM` — permutation invariance under a fixed atom renumbering.
- `M-EQUIVAR-ID` — `d(emb(orig), emb(orig))`; floor for "MUT distance honours its own metric axioms".

A complementary **feature-space noise floor** — `‖D(R)·coeff_orig − coeff_rot‖_F` per base — is computed once and persisted to `results/EXP-EQUIVAR/noise_floor.json`; the SUMMARY preamble shows the worst base. It's not directly comparable to MUT distances (different units, different pooling) but answers a separate question: is ElektroNN itself equivariant, and does its SH convention match e3nn's? Order-1 relative residuals there would point at the convention mismatch fix in [docs/strain_b_e3nn_central_atom.md §3](docs/strain_b_e3nn_central_atom.md). **Failures don't abort the run** — they surface as table values, so iteration on a new MUT stays unblocked.

**`Experiment.preamble` hook.** Optional `preamble(method_ids) -> str | None` on any experiment, called by `_write_summary_md` after the section heading and before the metrics table. EXP-EQUIVAR uses it for the feature-space noise-floor summary line; other experiments can opt in for any per-experiment context that doesn't fit the metric table.

**`Experiment.method_filter` hook.** Optional `method_filter(method) -> bool` on any experiment. The runner skips both embedding and experiment execution for any method where this returns False. EXP-EQUIVAR uses it to restrict S0 work to MUTs (`method.is_mut`); baselines never embed on S0. Other experiments inherit the implicit "applies to all methods" default.

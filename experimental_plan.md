...
**Decision at EXP-4 implementation time: defer.** The unstratified MoleculeACE route (reconstruct similar-pair cliffs, report M-DIST-POTENCY-RHO / M-CLIFF-AUC / M-SALI-DIST, plus the paired-bootstrap cross-method ρ test against B1–B6) is what shipped. The transformation classifier is a non-trivial chunk of cheminformatics (a few hundred LOC: SMARTS patterns for change-site detection, atom-level matching across the pair, charge-state and H-bond counting, vdW-volume estimation) with judgment-call rules (e.g. is -OH → -SH "electronic" or "bioisosteric"?). The R-group / ring / linker sub-analysis is likewise deferred: MoleculeACE pairs are similarity-defined rather than MMP-defined, so a clean transform-type label needs an `mmpdb` (or equivalent) pass that is not required for the headline metrics. Both can land as a follow-up without invalidating the unstratified results.
